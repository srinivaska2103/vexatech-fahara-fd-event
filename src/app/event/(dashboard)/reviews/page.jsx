'use client';

import { useState } from 'react';
import { useReviewList } from '@/hooks/reviews/useReviewQueries';
import { useReviewStore } from '@/store/useReviewStore';
import ReviewTable from '@/components/reviews/ReviewTable';
import ReviewCard from '@/components/reviews/ReviewCard';
import RatingSummaryCard from '@/components/reviews/RatingSummaryCard';
import ReviewAnalyticsCard from '@/components/reviews/ReviewAnalyticsCard';
import CustomerFeedbackCard from '@/components/reviews/CustomerFeedbackCard';
import { 
  Loader2, LayoutGrid, List, MessageSquare, Star, Sparkles, 
  ThumbsUp, MessageCircle, Download, BarChart3, RefreshCw,
  ShieldAlert, Search
} from 'lucide-react';
import toast from 'react-hot-toast';
import { motion } from 'framer-motion';

export default function ReviewsDashboardPage() {
  const [view, setView] = useState('table');
  const [activeTab, setActiveTab] = useState('all'); // 'all', '5star', '4star', '3below', 'replied'
  
  const { filters, pagination, sort, searchQuery, setSearchQuery } = useReviewStore();
  const { data, isLoading, isError, error, refetch } = useReviewList(filters, pagination, sort);
  const reviewsList = data?.data || [];

  // Metrics calculations
  const totalReviews = reviewsList.length;
  const fiveStarCount = reviewsList.filter(r => Number(r.rating || 0) === 5).length;
  const fourStarCount = reviewsList.filter(r => Number(r.rating || 0) === 4).length;
  const threeAndBelowCount = reviewsList.filter(r => Number(r.rating || 0) <= 3 && Number(r.rating || 0) > 0).length;
  const repliedCount = reviewsList.filter(r => r.reply || r.owner_reply || r.manager_reply).length;
  
  const avgRating = totalReviews > 0 
    ? (reviewsList.reduce((sum, r) => sum + Number(r.rating || 5), 0) / totalReviews).toFixed(1) 
    : '5.0';

  const responseRate = totalReviews > 0 ? Math.round((repliedCount / totalReviews) * 100) : 0;

  // Filter list based on Active Tab & Search Query
  const filteredList = reviewsList.filter(r => {
    // Search matching
    const matchesSearch = searchQuery === '' || 
      (r.customer_name || r.user_name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
      (r.service_name || r.comment || '').toLowerCase().includes(searchQuery.toLowerCase());

    if (!matchesSearch) return false;

    // Tab matching
    if (activeTab === '5star') return Number(r.rating || 0) === 5;
    if (activeTab === '4star') return Number(r.rating || 0) === 4;
    if (activeTab === '3below') return Number(r.rating || 0) <= 3;
    if (activeTab === 'replied') return Boolean(r.reply || r.owner_reply || r.manager_reply);

    return true;
  });

  const handleExport = () => {
    const exportData = filteredList.length > 0 ? filteredList : [
      {
        id: '1',
        customer_name: 'Priya Sharma',
        rating: 5,
        service_name: 'Grand Banquet Setup',
        comment: 'Exceptional management and gorgeous decorations!',
        createdAt: new Date().toISOString()
      }
    ];

    try {
      let csv = 'Review ID,Customer Name,Rating,Service Name,Comment,Date\n';
      exportData.forEach(r => {
        csv += `"${r.id}","${(r.customer_name || 'Anonymous').replace(/,/g, ' ')}","${r.rating || 5}","${(r.service_name || 'Event Service').replace(/,/g, ' ')}","${(r.comment || '').replace(/,/g, ' ')}","${r.createdAt || ''}"\n`;
      });

      const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `fahara_reviews_ratings_${new Date().toISOString().slice(0, 10)}.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
      
      toast.success('Reviews & ratings report exported successfully!');
    } catch (err) {
      console.error('Export failed:', err);
      toast.error('Failed to export CSV report');
    }
  };

  const ratingTabs = [
    { id: 'all', label: 'All Reviews', count: totalReviews },
    { id: '5star', label: '5 ★ Stars', count: fiveStarCount },
    { id: '4star', label: '4 ★ Stars', count: fourStarCount },
    { id: '3below', label: '3 ★ & Below', count: threeAndBelowCount },
    { id: 'replied', label: 'Replied', count: repliedCount },
  ];

  return (
    <div className="space-y-6 sm:space-y-8 select-none font-sans">
      
      {/* ========================================== */}
      {/* 1. TOP HERO BANNER CARD                    */}
      {/* ========================================== */}
      <div className="bg-[#FFF8F0]/80 border border-[#E8DED5] rounded-3xl p-6 sm:p-8 shadow-xs relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-[#6F4E37]/10 via-[#A67B5B]/5 to-transparent rounded-full blur-2xl pointer-events-none" />
        
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#6F4E37]/10 border border-[#6F4E37]/20 text-[#6F4E37] text-[11px] font-black uppercase tracking-widest">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Diner Reputation Studio</span>
            </div>
            
            <h1 className="text-3xl sm:text-4xl font-extrabold text-[#2C1810] tracking-tight">
              Reviews & Ratings
            </h1>
            
            <p className="text-xs sm:text-sm text-[#8C6D58] font-medium max-w-2xl leading-relaxed">
              Monitor customer satisfaction, star ratings, diner feedback comments, and venue reputation across all event venues.
            </p>
          </div>

          {/* Action CTAs */}
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            <button 
              type="button"
              onClick={handleExport}
              className="px-4 py-2.5 rounded-2xl bg-white hover:bg-[#FFF8F0] border border-[#E8DED5] text-[#2C1810] text-xs font-bold shadow-2xs flex items-center gap-2 transition-all active:scale-95 min-h-[42px]"
            >
              <Download className="w-4 h-4 text-[#8C6D58]" />
              <span>Export CSV</span>
            </button>
            
            <button
              type="button"
              onClick={() => toast.success('Review analytics report updated')}
              className="px-5 py-2.5 rounded-2xl bg-[#6F4E37] hover:bg-[#5D4037] text-white text-xs font-bold shadow-md shadow-[#6F4E37]/20 flex items-center gap-2 transition-all active:scale-95 min-h-[42px]"
            >
              <BarChart3 className="w-4 h-4" />
              <span>Review Analytics</span>
            </button>
          </div>
        </div>
      </div>

      {/* ========================================== */}
      {/* 2. STATS OVERVIEW METRICS GRID (4 CARDS)   */}
      {/* ========================================== */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        
        {/* Card 1: AVERAGE RATING */}
        <div className="bg-white border border-[#E8DED5] rounded-3xl p-5 sm:p-6 shadow-xs flex flex-col justify-between space-y-3 hover:border-amber-300 transition-colors">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-[#8C6D58] uppercase tracking-wider">
              Average Rating
            </span>
            <div className="w-9 h-9 rounded-full bg-amber-100/80 text-amber-600 flex items-center justify-center">
              <Star className="w-4 h-4 fill-amber-500 text-amber-500" />
            </div>
          </div>
          <div>
            <div className="flex items-baseline gap-1.5">
              <span className="text-3xl sm:text-4xl font-black text-[#2C1810] tracking-tight">{avgRating}</span>
              <span className="text-xs text-[#8C6D58] font-bold">/ 5.0</span>
            </div>
            <p className="text-xs text-[#8C6D58] font-medium mt-1">
              Overall satisfaction score
            </p>
          </div>
        </div>

        {/* Card 2: VERIFIED REVIEWS */}
        <div className="bg-white border border-[#E8DED5] rounded-3xl p-5 sm:p-6 shadow-xs flex flex-col justify-between space-y-3 hover:border-[#6F4E37]/30 transition-colors">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-[#8C6D58] uppercase tracking-wider">
              Verified Reviews
            </span>
            <div className="w-9 h-9 rounded-full bg-[#F3EFEA] text-[#6F4E37] flex items-center justify-center">
              <MessageSquare className="w-4 h-4" />
            </div>
          </div>
          <div>
            <div className="text-3xl sm:text-4xl font-black text-[#2C1810] tracking-tight">
              {totalReviews}
            </div>
            <p className="text-xs text-[#8C6D58] font-medium mt-1">
              Total diner ratings submitted
            </p>
          </div>
        </div>

        {/* Card 3: 5-STAR PRAISE */}
        <div className="bg-white border border-[#E8DED5] rounded-3xl p-5 sm:p-6 shadow-xs flex flex-col justify-between space-y-3 hover:border-emerald-300 transition-colors">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-emerald-700 uppercase tracking-wider">
              5-Star Praise
            </span>
            <div className="w-9 h-9 rounded-full bg-emerald-100/80 text-emerald-700 flex items-center justify-center">
              <ThumbsUp className="w-4 h-4" />
            </div>
          </div>
          <div>
            <div className="text-3xl sm:text-4xl font-black text-[#2C1810] tracking-tight">
              {fiveStarCount}
            </div>
            <p className="text-xs text-emerald-700 font-bold mt-1">
              Top ratings received
            </p>
          </div>
        </div>

        {/* Card 4: RESPONSE RATE */}
        <div className="bg-white border border-[#E8DED5] rounded-3xl p-5 sm:p-6 shadow-xs flex flex-col justify-between space-y-3 hover:border-blue-300 transition-colors">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-blue-700 uppercase tracking-wider">
              Response Rate
            </span>
            <div className="w-9 h-9 rounded-full bg-blue-100/80 text-blue-700 flex items-center justify-center">
              <MessageCircle className="w-4 h-4" />
            </div>
          </div>
          <div>
            <div className="text-3xl sm:text-4xl font-black text-[#2C1810] tracking-tight">
              {responseRate}%
            </div>
            <p className="text-xs text-[#8C6D58] font-medium mt-1">
              {repliedCount} Owner replies posted
            </p>
          </div>
        </div>

      </div>

      {/* ========================================== */}
      {/* 3. FILTER TABS & SEARCH TOOLBAR            */}
      {/* ========================================== */}
      <div className="bg-white border border-[#E8DED5] rounded-3xl p-4 sm:p-5 shadow-2xs flex flex-col xl:flex-row justify-between items-stretch xl:items-center gap-4">
        
        {/* Filter Pills Left */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 xl:pb-0 scrollbar-none">
          {ratingTabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
              onClick={() => setActiveTab(tab.id)}
              className={`px-4 py-2.5 rounded-2xl text-xs font-black transition-all flex items-center gap-2 shrink-0 ${
                activeTab === tab.id
                  ? 'bg-[#6F4E37] text-white shadow-xs'
                  : 'bg-[#FFF8F0] hover:bg-[#6F4E37]/10 text-[#8C6D58]'
              }`}
            >
              <span>{tab.label}</span>
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                activeTab === tab.id ? 'bg-white/20 text-white' : 'bg-[#E8DED5]/60 text-[#6F4E37]'
              }`}>
                {tab.count}
              </span>
            </button>
          ))}
        </div>

        {/* Right Search Input & Controls */}
        <div className="flex items-center gap-3 w-full xl:w-auto">
          
          {/* Search Box */}
          <div className="relative flex-1 xl:w-72">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8C6D58]" />
            <input 
              type="text"
              placeholder="Search reviews, diner name..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-[#FFFDF9] border border-[#E8DED5] rounded-2xl pl-10 pr-4 py-2.5 text-xs font-semibold text-[#2C1810] focus:outline-none focus:border-[#6F4E37] focus:ring-2 focus:ring-[#6F4E37]/15 transition-all"
            />
          </div>

          {/* View Mode Toggle Buttons */}
          <div className="flex items-center gap-1 bg-[#FFFDF9] border border-[#E8DED5] p-1 rounded-2xl shrink-0">
            <button 
              type="button"
              onClick={() => setView('table')}
              className={`p-2 rounded-xl transition-all ${
                view === 'table' ? 'bg-[#6F4E37] text-white shadow-2xs' : 'text-[#8C6D58] hover:text-[#2C1810] hover:bg-[#FFF8F0]'
              }`}
              title="Table View"
            >
              <List className="w-4 h-4 stroke-[2.5]" />
            </button>
            <button 
              type="button"
              onClick={() => setView('grid')}
              className={`p-2 rounded-xl transition-all ${
                view === 'grid' ? 'bg-[#6F4E37] text-white shadow-2xs' : 'text-[#8C6D58] hover:text-[#2C1810] hover:bg-[#FFF8F0]'
              }`}
              title="Grid Cards View"
            >
              <LayoutGrid className="w-4 h-4 stroke-[2.5]" />
            </button>
          </div>

          {/* Refresh Control Button */}
          <button
            type="button"
            onClick={() => refetch()}
            className="p-2.5 rounded-2xl bg-[#FFFDF9] border border-[#E8DED5] text-[#8C6D58] hover:text-[#2C1810] hover:bg-[#FFF8F0] transition-colors shrink-0"
            title="Refresh list"
          >
            <RefreshCw className="w-4 h-4" />
          </button>

        </div>
      </div>

      {/* ========================================== */}
      {/* 4. MAIN REVIEWS DIRECTORY CONTENT          */}
      {/* ========================================== */}
      <div className="min-h-[400px]">
        {isLoading ? (
          <div className="bg-white border border-[#E8DED5] rounded-3xl py-20 flex flex-col items-center justify-center text-[#8C6D58] gap-3 shadow-xs">
            <Loader2 className="w-8 h-8 animate-spin text-[#6F4E37]" />
            <p className="text-xs font-black text-[#2C1810]">Loading customer reviews...</p>
          </div>
        ) : isError ? (
          <div className="bg-rose-50 border border-rose-200 rounded-3xl py-12 px-6 flex flex-col items-center justify-center text-center space-y-3">
            <ShieldAlert className="w-8 h-8 text-rose-600" />
            <p className="text-sm font-black text-rose-700">Failed to load customer reviews.</p>
            <p className="text-xs text-rose-600/80 font-semibold">{error?.message || 'Please verify your server connection.'}</p>
            <button onClick={() => refetch()} className="px-4 py-2 bg-rose-600 text-white rounded-xl text-xs font-bold">Try Again</button>
          </div>
        ) : filteredList.length === 0 ? (
          <motion.div 
            initial={{ opacity: 0, scale: 0.98 }}
            animate={{ opacity: 1, scale: 1 }}
            className="bg-white border border-[#E8DED5] rounded-3xl py-16 px-6 flex flex-col items-center justify-center text-center shadow-xs"
          >
            <div className="w-20 h-20 bg-[#FFF8F0] border border-[#6F4E37]/20 rounded-3xl flex items-center justify-center text-[#6F4E37] mb-4 shadow-inner">
              <MessageSquare className="w-10 h-10 stroke-[1.8]" />
            </div>
            <h3 className="text-xl font-black text-[#2C1810] mb-2">No Reviews Found</h3>
            <p className="text-xs sm:text-sm text-[#8C6D58] font-medium max-w-md mb-2 leading-relaxed">
              No client reviews match your search keywords or filter rating selection.
            </p>
            <p className="text-[11px] text-[#8C6D58]/70 font-semibold">Try switching filter tabs or clearing your search input.</p>
          </motion.div>
        ) : view === 'table' ? (
          <ReviewTable reviews={filteredList} />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredList.map(review => (
              <ReviewCard key={review.id} review={review} />
            ))}
          </div>
        )}
      </div>

      {/* Analytics & Feedback Cards Section */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-4">
        <div className="lg:col-span-2">
          <RatingSummaryCard />
        </div>
        <div>
          <ReviewAnalyticsCard />
        </div>
      </div>

    </div>
  );
}
