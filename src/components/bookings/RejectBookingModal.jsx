...
import { useRejectBooking } from '@/hooks/bookings/useBookingMutations';
...
export default function RejectBookingModal() {
  ...
  const { isOpen, bookingId } = modals.reject;
  ...
  const onSubmit = (data) => {
    rejectMutation.mutate({ id: bookingId, reason: data.reason }, {
      onSuccess: () => {
        closeModal('reject');
        reset();
      }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
      <div className="bg-surface border border-white/10 rounded-3xl w-full max-w-md shadow-2xl relative overflow-hidden">
        ...
        <div className="p-6 md:p-8">
          ...
          <h3 className="text-xl font-bold text-text mb-2">Reject Booking</h3>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <label className="block text-xs font-semibold text-text/70 mb-2 uppercase tracking-wider">Rejection Reason</label>
              <textarea
                {...register('reason')}
                rows={4}
